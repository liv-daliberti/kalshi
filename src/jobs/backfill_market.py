...
from ..db.db import dec, get_state, parse_ts_iso, set_state
from ..core.time_utils import normalize_strike_period
from ..kalshi.kalshi_sdk import get_market_candlesticks
from ..queue.work_queue import QueueConfig, QueuePublisher, enqueue_job
...
def _interval_minutes_for_strike(
    ...
    :rtype: int
    """
    strike_period = normalize_strike_period(strike_period) or ""
...
def _window_is_filled(
    request: BackfillRangeRequest,
) -> bool:
    """Return True if existing data covers the target window."""
    if request.force_full:
        return False
    return not _build_backfill_ranges(request)
...
def _insert_candle(cur, market_ticker: str, period_minutes: int, candle: dict) -> None:
    ...
    cur.execute(
        """
        INSERT INTO market_candles_latest(
          market_ticker, period_interval_minutes, end_period_ts, start_period_ts,
          open, high, low, close, volume, open_interest, raw
        )
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        ON CONFLICT (market_ticker) DO UPDATE SET
          period_interval_minutes = EXCLUDED.period_interval_minutes,
          end_period_ts = EXCLUDED.end_period_ts,
          start_period_ts = EXCLUDED.start_period_ts,
          open = EXCLUDED.open,
          high = EXCLUDED.high,
          low = EXCLUDED.low,
          ...
          raw = EXCLUDED.raw,
          updated_at = NOW()
        WHERE market_candles_latest.end_period_ts IS NULL
           OR EXCLUDED.end_period_ts >= market_candles_latest.end_period_ts
        """,
        (
            market_ticker,
            period_minutes,
...
def _apply_cursor_window(
    ...
    force_full: bool,
    *,
    state_cursor_s: Optional[int] = None,
    existing_max_s: Optional[int] = None,
) -> tuple[int, Optional[int], Optional[int]]:
    if existing_max_s is None:
        _, existing_max_s = _fetch_candle_bounds(
            conn,
            market_ticker,
            period_minutes,
        )
    if state_cursor_s is None:
...
def _build_market_backfill_window(
    ...
    existing_min_s, existing_max_s = _fetch_candle_bounds(
        conn,
        market_ticker,
        period_minutes,
    )
    state_cursor_s = _load_backfill_cursor(conn, market_ticker, period_minutes)
    if _market_is_closed(context.market):
        filled_request = BackfillRangeRequest(
            target_start_s=start_s,
            target_end_s=end_s,
            existing_min_s=existing_min_s,
            existing_max_s=existing_max_s,
            period_seconds=period_minutes * 60,
            force_full=force_full,
        )
        if _window_is_filled(filled_request):
            last_cursor_s = state_cursor_s
            if existing_max_s is not None and (
                last_cursor_s is None or existing_max_s > last_cursor_s
            ):
                last_cursor_s = existing_max_s

"""WebSocket ingestion service entrypoint."""
...
def _ensure_env() -> None:
    os.environ["KALSHI_RUN_MODE"] = "ws"
    os.environ.setdefault("SERVICE_ROLE", "ws")
    os.environ.setdefault("DB_INIT_SCHEMA", "0")


def main() -> None:
    """Entrypoint for running the websocket ingestion service."""
    _ensure_env()
    assert_env_isolated(
        "ws",
        forbidden_prefixes=("RAG_", "AZURE_", "PREDICTION_", "DATABASE_URL_"),
    )
    start_health_server("ws")
